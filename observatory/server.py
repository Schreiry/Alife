...
@app.get("/api/status")
async def status():
    return JSONResponse(runner().with_snapshot(live_snapshot))
...
@app.get("/api/events")
async def events(limit: int = 80, kind: Optional[str] = None):
    r = runner()
    tel = r.sim.telemetry
    if tel is not None:
        return JSONResponse({
...
async def get_creature(cid: int):
    r = runner()
