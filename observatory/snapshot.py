...
def live_snapshot(sim) -> Dict[str, Any]:
    ...
    if n:
        ...
        # Energy fraction (0..1) for the "hunger" view layer. View-only.
        ef = np.clip(
            store.energy[alive_idx] / np.maximum(1.0, store.max_energy[alive_idx]),
            0.0, 1.0,
        )
        energy_frac = np.round(ef, 3).astype(np.float32).tolist()
        ...
        colors_r = []; colors_g = []; colors_b = []; energy_frac = []
    ...
    for clan in world.clans.values():
        clans_payload.append({
            ...
            "wars": len(clan.at_war),
        })

    # Recent positioned event markers, fresh enough to still be fading in.
    ttl = config.SIGNAL_TTL_TICKS
    tick_now = world.tick
    signals = [s for s in world.signals if 0 <= tick_now - s["tick"] <= ttl]
...
_PULSE_STRESS = {"attack", "flee", "defend"}
_PULSE_ACTIVE = {
    "eat", "seek_food", "reproduce", "seek_mate", "seek_distant_mate",
    "claim_territory", "migrate", "follow_clan",
}


def _compute_pulse(creature) -> Dict[str, Any]:
    """Derived vital signal — heart rate + a color state read from the
    creature's current behavior and physiology. Observation-only (computed on
    demand for the inspector); it never feeds back into the tick so it can't
    destabilize a running world."""
    ef = creature.energy / max(1.0, creature.max_energy)
    hf = creature.health / max(1.0, creature.max_health)
    act = creature.last_action or ""
    metab = creature.genome.normalized("metabolism_speed")
    base = 50.0 + 48.0 * metab                       # resting rate from metabolism

    if act in _PULSE_STRESS:
        state, mult, color = "stress", 1.85, [255, 78, 60]
    elif act in _PULSE_ACTIVE:
        state, mult, color = "active", 1.32, [120, 224, 122]
    else:
        state, mult, color = "calm", 0.86, [108, 168, 255]

    rate = base * mult * (1.0 + 0.28 * (1.0 - ef))   # hunger drives it up
    if hf < 0.20:                                     # near death overrides
        state, color = "critical", [255, 44, 44]
        rate *= 1.12
    return {
        "rate": int(max(32, min(208, rate))),
        "state": state,
        "color": color,
    }
...
def creature_detail(sim, cid: int) -> Optional[Dict[str, Any]]:
    ...
    return {
        "id": creature.id,
        "pulse": _compute_pulse(creature),
        "brain": brain_net.build(genome),
        "x": float(creature.x),
        "y": float(creature.y),
