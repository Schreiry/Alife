...
class Telemetry:
    ...
    def flush(self) -> None:
        ...
        try:
            ...
            # Telemetry must NEVER break the simulation or kill its own writer
            # thread. A narrow sqlite3.Error catch let other errors (disk full,
            # encoding, OS-level) crash the flusher silently, which froze the
            # charts while the sim kept running. Catch everything and log.
            import sys
    ...
    def metric_history(self, key: str, limit: int = 500) -> List[Tuple[int, float]]:
        # Order by rowid (insertion order), NOT tick: the db is persistent
        # across runs, so a fresh/loaded sim writes rows whose ticks may be
        # lower than a previous longer run's. Ordering by tick would surface
        # the stale old-run tail forever; the active run's rows are always the
        # most recently inserted, so rowid DESC gives the live series.
        with sqlite3.connect(self.db_path, timeout=2.0) as con:
            rows = con.execute(
