...
def load(simulation, path: str = config.SAVE_PATH) -> bool:
    ...
    for clan_data in payload.get("clans", []):
        ...
        # at_war / allied are derived from relations (not serialized) so a
        # reloaded world doesn't re-announce wars it already declared.
        clan.at_war = {k for k, v in clan.relations.items() if v <= config.WAR_THRESHOLD}
        clan.allied = {k for k, v in clan.relations.items() if v >= config.ALLIANCE_THRESHOLD}
